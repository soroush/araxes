...
namespace araxes {
    class shared_mutex {
            ...
            static const uint32_t write_entered_ = 1U << (sizeof(uint32_t)*CHAR_BIT - 1);
            static const uint32_t n_readers_ = ~write_entered_;
    ...
    class upgrade_mutex {
            ...
            static const unsigned write_entered_ = 1U << (sizeof(uint32_t)*CHAR_BIT - 1);
    ...
    template <class Clock, class Duration>
    bool upgrade_mutex::try_lock_until(const std::chrono::time_point<Clock, Duration>& abs_time) {
        std::unique_lock<std::mutex> lk(mut_);
    ...
    template <class Clock, class Duration>
    bool upgrade_mutex::try_lock_shared_until(const std::chrono::time_point<Clock, Duration>& abs_time) {
        std::unique_lock<std::mutex> lk(mut_);
        if ((state_ & write_entered_) || (state_ & n_readers_) == n_readers_) {
            while (true) {
                std::cv_status status = gate1_.wait_until(lk, abs_time);
